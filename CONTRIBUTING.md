...
Thank you for your interest in contributing to Argus Panoptes! This guide covers everything you need to get started.
...
We use [pytest](https://docs.pytest.org/) with [pytest-asyncio](https://github.com/pytest-dev/pytest-asyncio) for async tests.
...
- Use the fixtures from `tests/conftest.py` (e.g., `app_client`, `async_session`)
- Follow the existing test patterns -- see `tests/unit/test_security.py` for examples
...
2. **HTMX targets partials** -- each `hx-get` hits a route that returns a template fragment
3. **Hot-reload is foundational** -- the ConfigManager watches `config/argus.yaml` for changes
4. **Dual database support** -- code must work with both SQLite and PostgreSQL
5. **Type everything** -- all function signatures have type hints, mypy strict mode

### Database

- SQLite for development (default, zero setup)
- PostgreSQL for production (set `database.url` in config or `DATABASE_URL` env var)
- Alembic manages schema migrations for both backends
- Use `render_as_batch=True` in Alembic for SQLite ALTER TABLE compatibility

## Pull Request Process

1. **Fork and branch** -- create a feature branch from `main`
   - Branch naming: `feature/<summary>`, `bugfix/<summary>`, `docs/<summary>`

2. **Write tests first** -- add failing tests before implementing features
...
5. **Write a clear commit message**:
   ...
   - Example: `feat: add API key authentication for ingestion endpoints`
...
7. **Address review feedback** -- push new commits (do not force-push)
